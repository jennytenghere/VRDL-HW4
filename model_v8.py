...
class PromptGenBlock(nn.Module):
    def __init__(self,prompt_dim=128,prompt_len=5,prompt_size=96,lin_dim=192):
        ...
        self.project = nn.Sequential(
            nn.Conv2d(lin_dim, prompt_dim, kernel_size=3, padding=1),
            nn.ReLU(),
    ...
    def forward(self,x):
        ...
        # simply used self.project to generate the prompt
        prompt = self.project(x)

        # emb = x.mean(dim=(-2,-1))
        # prompt_weights = F.softmax(self.linear_layer(emb),dim=1)
        # # torch.Size([4, 5])
        # prompt = prompt_weights.unsqueeze(-1) * self.prompt_param.unsqueeze(0).repeat(B,1,1,1).squeeze(1)
        # # torch.Size([4, 5, 320])
        # prompt = torch.sum(prompt,dim=1)
        # # torch.Size([4, 320])
        # prompt = prompt.unsqueeze(2).unsqueeze(3).repeat(1, 1, H, W) 
